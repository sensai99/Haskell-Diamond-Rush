module Main(main) where
    ...
    timer_dis t = scale 0.3 0.3 $ translate (400*3.5) (400*3.5) $ text (show t)

    level_dis :: [Char] -> Picture
    level_dis x = scale 0.3 0.3 $ translate (-800*3.5) (400*3.5) $ text (show x)
    
    diamond :: [Float] -> [Float] -> [Picture]
    diamond [] [] = []
    diamond (x:xs) (y:ys) = [translate x y $ color green $ rectangleSolid 10 10] ++ diamond xs ys      

    render :: Maze -> Picture
    render maze = pictures((squares maze a b c ) ++ [player x y] ++ [(timer_dis t)] ++ [(level_dis level)] ++ diamond [dx] [dy])
        where 
            (x,y) = playerLoc maze
    ...
    walls_position :: (Float,Float) -> Int -> [(Float,Float,Int)] --left is 0 right is 1 top is 0 bottom is 1
    walls_position (a,b) 0 = []
    walls_position (a,b) 1 = [(a-45,b,1)] -- 1 is vertical 0 is horizontal
    walls_position (a,b) 2 = [(a,b+45,0)]
    walls_position (a,b) 3 = [(a+45,b,1)]
    walls_position (a,b) 4 = [(a,b-45,0)]
    walls_position (a,b) 5 = [(a,b+45,0)] ++ [(a-45,b,1)]
    walls_position (a,b) 6 = [(a,b+45,0)] ++ [(a+45,b,1)]
    walls_position (a,b) 7 = [(a,b-45,0)] ++ [(a+45,b,1)]
    walls_position (a,b) 8 = [(a,b-45,0)] ++ [(a-45,b,1)]
    walls_position (a,b) 9 = [(a-45,b,1)] ++ [(a+45,b,1)]
    walls_position (a,b) 10 = [(a,b+45,0)] ++ [(a,b-45,0)]
    walls_position (a,b) 11 = [(a-45,b,1)] ++ [(a,b+45,0)] ++ [(a+45,b,1)]
    walls_position (a,b) 12 = [(a,b-45,0)] ++ [(a,b+45,0)] ++ [(a+45,b,1)]
    walls_position (a,b) 13 = [(a,b-45,0)] ++ [(a,b+45,0)] ++ [(a-45,b,1)]
    walls_position (a,b) 14 = [(a,b-45,0)] ++ [(a-45,b,1)] ++ [(a+45,b,1)]
    ...
    wallcollision :: (Float,Float) -> (Float,Float,Int) -> (Float,Float) -> Bool
    wallcollision (x,y) (a,b,c) (vx,vy) = collision
        where
            collision =
    ...
    all_walls :: Int -> Maze-> [(Float,Float,Int)]
    all_walls 0 game = [] 
    ...
    shouldmove :: Maze -> (Float,Float) -> (Float,Float) -> [(Float,Float,Int)] -> Bool
    ...
    collected_diamond :: (Float,Float) -> (Float,Float) -> Bool -- returns true if diamond is collected
    collected_diamond (px,py) (dx,dy) = 
        if ((px - dx <= 10) && (px - dx >= -10)) then -- player before diamond
            if ((py - dy <= 10) && (py - dy >= -10)) then True else False
        else False    

    moveplayer :: Float -> Maze -> Maze
    moveplayer seconds game = 
        if (collected_diamond (x,y) (xout,yout) ) then -- returns true if diamond is collected
            if l == 1.1 then level2 world1 else game -- level upgrade
        else if (x + vx*seconds >= x_max || x + vx*seconds <= x_min || y + vy*seconds >= y_max || y + vy*seconds <= y_min ) then game {timer = (timer game) - seconds}  -- out of maze
        else    
            if (timer game) > 0 then
                if (shouldmove game (x + vx*seconds,y + vy*seconds) (vx,vy) (all_walls last game))  then 
